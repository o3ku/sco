...
$versionExe = Join-Path $Root 'apps\sco\0.4.0\sco.exe'
...
foreach ($path in @(
    ...
    (Join-Path $Root 'apps\sco\0.4.0\manifest.json'),
    (Join-Path $Root 'apps\sco\0.4.0\install.json')
)) {
    if (!(Test-Path $path)) {
...
if ($scoManifest.version -ne '0.4.0' -or $scoManifest.bin -ne 'sco.exe') {
...
$versionManifestWriteTime = (Get-Item (Join-Path $Root 'apps\sco\0.4.0\manifest.json')).LastWriteTimeUtc
...
if ((Get-Item (Join-Path $Root 'apps\sco\0.4.0\manifest.json')).LastWriteTimeUtc -ne $versionManifestWriteTime) {
