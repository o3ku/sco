...
$versionExe = Join-Path $Root 'apps\sco\0.6.3\sco.exe'
...
foreach ($path in @(
    ...
    (Join-Path $Root 'apps\sco\0.6.3\manifest.json'),
    (Join-Path $Root 'apps\sco\0.6.3\install.json')
)) {
    if (!(Test-Path $path)) {
...
if ($scoManifest.version -ne '0.6.3' -or $scoManifest.bin -ne 'sco.exe') {
...
if ($listOutput -notmatch 'sco\s+0\.6\.3') {
...
if ($scoopListOutput -notmatch 'sco\s+0\.6\.3') {
...
$versionManifestWriteTime = (Get-Item (Join-Path $Root 'apps\sco\0.6.3\manifest.json')).LastWriteTimeUtc
...
if ((Get-Item (Join-Path $Root 'apps\sco\0.6.3\manifest.json')).LastWriteTimeUtc -ne $versionManifestWriteTime) {
